 ...
 *
 * The IPC handlers under test still read electron's `app.getPath`, while the shared
 * provisioning core reads the AppEnvironment port. Setting only the electron mock leaves the
 * port on the global test setup's own temp directory, so a provision writes its runtime record
 * to one place and the assertions read another — which surfaces as an empty runtime list, not
 * as an error.
 */
